...
export class ScrollViewCmpt extends ScrollView {
    ...
    updateItemInfo(item, idx) {
        ...
        if (script && this.m_data && this.m_data.list && this.m_data.list[idx]) {
    ...
    scrolling() {
        ...
        // 检查关键对象是否有效，避免在组件销毁后继续执行
        if (!this.node || !this.node.isValid || !this.scrollItem || !this.scrollItem.isValid) {
            return;
        }
        ...
        for (let i = 0; i < this.itemArr.length; i++) {
            ...
            // 检查item及其父节点是否有效
            if (!item || !item.isValid || !item.parent || !item.parent.isValid) {
                continue;
            }
            ...
            let itemParentTransform = item.parent.getComponent(UITransform);
            let thisParentTransform = this.node.parent.getComponent(UITransform);
            let thisTransform = this.node.getComponent(UITransform);
            let scrollItemTransform = this.scrollItem.getComponent(UITransform);
            
            // 检查所有组件是否存在
            if (!itemParentTransform || !thisParentTransform || !thisTransform || !scrollItemTransform) {
                continue;
            }
            
            let worldPos = itemParentTransform.convertToWorldSpaceAR(pos);
            let thisPos = thisParentTransform.convertToWorldSpaceAR(this.node.getPosition());
            let gap = thisTransform.height / 2;
            
            if (worldPos.y >= thisPos.y + gap + scrollItemTransform.height) {
            ...
            let finalItemParentTransform = item.parent.getComponent(UITransform);
            if (!finalItemParentTransform) {
                continue;
            }
            
            worldPos = finalItemParentTransform.convertToWorldSpaceAR(pos);
            if (this.bottomY == -100000) {
                this.bottomY = thisPos.y - gap - scrollItemTransform.height
            }
        }
    }
}
